...
function init(){
	// Set up scene
	container = document.getElementById('cut_container');
...
function setupRenderer(){
	...
	renderer.setSize(window.innerWidth/2,window.innerHeight);
	cut_container.appendChild(renderer.domElement);
}

function setupCamera(){
	camera = new THREE.PerspectiveCamera(
		45,         // Field of view
		window.innerWidth/2/window.innerHeight,  // Aspect ratio
		.1,         // Near
		10000       // Far
	);
	scene.add(camera);
...
function setupGeometry(){
	...
	drawMesh(extrudePiece(head,extrusion_distance,extrusion_direction),"blue");
...
function makeShape(geometries){
	...
	for (var i = 0; i < vertices.length; i++){
		drawPoint(vertices[i],i/vertices.length);
	}
