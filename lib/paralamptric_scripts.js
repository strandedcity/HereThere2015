 var solar_tilt=45, lamp_height=12, lamp_width=3, draw_construction = false, sheet_thickness=1/12, sheet_width=192, sheet_height=384, sheets=[], sheet_count = 0, ui_throttle=350;

 // mutable vars accessible to the global scope
 ...
 $(window).load(function(){
	...
	$('#map_pin').draggable({
		containment: 'parent'
		, drag: $.throttle(ui_throttle, dragMapHandler)
	})
...
function dragMapHandler (event, ui) {
	...
	// console.log('relative pos of pointer in map', relative_pos)

	var coors = get_geo_coordinates_from_relative_coordinates(
				relative_pos.x
				, relative_pos.y)

	// console.log('coordinates i think the pin is at:', coors)

	// get solar noon from map angle
    solarNoon = get_solar_noon(coors.lat, coors.lng)

	// console.log('what angle i think solarNoon is', solarNoon)

    // 0 tilt angle == panel is facing straight up, parallel to the earth
    solar_tilt = Math.abs(90-solarNoon)
    updateScenes()
}
...
function get_geo_coordinates_from_relative_coordinates (x,y) {
	...
	var lat = y.map(0,100,80,-80)
	var lng = x.map(0,100,-171,176)
