...
var rib_geometry, skin_geometry;
var skin_width = 3;
...
function drawCutGeometry(){
	...
	var pt = new THREE.Vector3(0,-12,0);
	var rect = getRectangle(pt,skin_width,skin_length);
	
	var stipples = [];
	stipples.push(getStipple(pt,getLength(skin_geometry[0]),getLength(skin_geometry[1])));
	stipples.push(getStipple(pt,getLength(skin_geometry[0])+getLength(skin_geometry[1])+getLength(skin_geometry[2]),getLength(skin_geometry[3])));
	...
	for (var i = 0; i < rib_geometry.length; i++){
		drawLine(cut_scene,rib_geometry[i],"black");
	}
	cut_renderer.render(cut_scene,cut_camera);
}
