...
function clearScene(scene){
	for (var i = scene.children.length - 1; i >= 0 ; i -- ) {
		...
		if (!THREE.PerspectiveCamera.prototype.isPrototypeOf(obj) && !THREE.HemisphereLight.prototype.isPrototypeOf(obj)&& !THREE.DirectionalLight.prototype.isPrototypeOf(obj)) {
			scene.remove(obj);
		}
	}
	animate();
...
function exportSVG() {
	...
	var svgfile = XMLS.serializeToString(cut_renderer.domElement);
	downloadBlob(svgfile,"TheTwo");
}
...
function drawCutGeometry(){
	...
	// Draw skin
	drawLine(cut_scene,skin_object.geometry,"black",[0,0,0]);
	...
	for (var i = 0; i < stipples.length; i++){
		for (var j = 0; j < stipples[i].length; j++){
			drawLine(cut_scene,stipples[i][j],"black",[0,0,0]);
		}
	}
	
	// Draw nubbin holes
	var nubbin_holes = [];
	nubbin_holes.push(getRectangle(skin_object.geometry.vertices[0].clone().add(new THREE.Vector3(SKIN_WIDTH/2-SHEET_THICKNESS/2,-getLength(skin_object.pieces[0])*skin_object.nubbin_ts[0]+NUBBIN_LENGTH/2,0)),SHEET_THICKNESS,NUBBIN_LENGTH));
	nubbin_holes.push(getRectangle(skin_object.geometry.vertices[0].clone().add(new THREE.Vector3(SKIN_WIDTH/2-SHEET_THICKNESS/2,-getLength(skin_object.pieces[0])*skin_object.nubbin_ts[1]+NUBBIN_LENGTH/2,0)),SHEET_THICKNESS,NUBBIN_LENGTH));
	nubbin_holes.push(getRectangle(skin_object.geometry.vertices[3].clone().add(new THREE.Vector3(SKIN_WIDTH/2-SHEET_THICKNESS/2,getLength(skin_object.pieces[skin_object.pieces.length-1])*skin_object.nubbin_ts[0]+NUBBIN_LENGTH/2,0)),SHEET_THICKNESS,NUBBIN_LENGTH));
	nubbin_holes.push(getRectangle(skin_object.geometry.vertices[3].clone().add(new THREE.Vector3(SKIN_WIDTH/2-SHEET_THICKNESS/2,getLength(skin_object.pieces[skin_object.pieces.length-1])*skin_object.nubbin_ts[1]+NUBBIN_LENGTH/2,0)),SHEET_THICKNESS,NUBBIN_LENGTH));
	for (var i = 0; i < nubbin_holes.length; i++){
		drawLine(cut_scene,nubbin_holes[i],"black",[0,0,0]);
	}

	// Draw rib
	drawLine(cut_scene,rib_object.geometry,"black",[0,0,0]);
	
	// Draw cut sheets
	for (var i = 0; i < cut_sheets.length; i++){
		drawLine(cut_scene,cut_sheets[i],"gray",[0,0,0]);
	}
...
function drawModelGeometry(solar_tilt,lamp_height,draw_construction){
	...
	movement = [0,lamp_height,0];
	/* Draw Construction */
	if (draw_construction){
		drawPoint(model_scene,neckCircle_center,construction_color,movement);
		drawCircle(model_scene,neckCircle,construction_color,movement);
		drawPoint(model_scene,topOfBody_pt,construction_color,movement);
		drawTangent(model_scene,topOfBody_pt,topOfBody_tan.clone().negate(),construction_color,movement);
		drawPoint(model_scene,topOfNeck_pt,construction_color,movement);
		drawTangent(model_scene,topOfNeck_pt,topOfNeck_tan.clone().negate(),construction_color,movement);
		drawPoint(model_scene,topOfHead_pt,construction_color,movement);
		drawPoint(model_scene,topOfBelly_pt,construction_color,movement);
		drawPoint(model_scene,bellyCircle_center,construction_color,movement);
		drawCircle(model_scene,bellyCircle,construction_color,movement);
		drawPoint(model_scene,bottomOfBelly_pt,construction_color,movement);
		drawTangent(model_scene,bottomOfBelly_pt,bottomOfBelly_tan,construction_color,movement);
	}
	
	var extrusion_direction = new THREE.Vector3(0,0,-1);
	var skin_color = 'rgb(230,230,230)';
	var head_extrusion = extrudePiece(head,SKIN_WIDTH,extrusion_direction,true);
	var neck_extrusion = extrudePiece(neck,SKIN_WIDTH,extrusion_direction,true);
	var body_extrusion = extrudePiece(body,SKIN_WIDTH,extrusion_direction,true);
	var belly_extrusion = extrudePiece(belly,SKIN_WIDTH,extrusion_direction,true);
	var tail_extrusion = extrudePiece(tail,SKIN_WIDTH,extrusion_direction,true);
	// moveGeometry(head_extrusion,movement);
	// moveGeometry(neck_extrusion,movement);
	// moveGeometry(body_extrusion,movement);
	// moveGeometry(belly_extrusion,movement);
	// moveGeometry(tail_extrusion,movement);
	drawMesh(model_scene,head_extrusion,skin_color,movement,true);
	drawMesh(model_scene,neck_extrusion,skin_color,movement,true);
	drawMesh(model_scene,body_extrusion,skin_color,movement,true);
	drawMesh(model_scene,belly_extrusion,skin_color,movement,true);
	drawMesh(model_scene,tail_extrusion,skin_color,movement,true);
	...
	//moveGeometry(rib_extrusion,[0,lamp_height,-SHEET_THICKNESS/2]);
	drawMesh(model_scene,rib_extrusion,skin_color,[0,lamp_height,-SHEET_THICKNESS/2],false);
	...
	skin_object.geometry = getRectangle(pt,SKIN_WIDTH,skin_length);
...
function moveGeometry(geometry,movement){
	...
	moved_geometry.applyMatrix(new THREE.Matrix4().makeTranslation(movement[0],movement[1],movement[2]));
...
function fillet(line1,line2,t,draw_construction){
	...
	if (draw_construction){
		drawLine(model_scene,line3,construction_color,movement);
		drawPoint(model_scene,cPt,construction_color,movement);
		drawPoint(model_scene,pt1,construction_color,movement);
		drawPoint(model_scene,pt2,construction_color,movement);
		drawCircle(model_scene,getCircle(cPt,r),construction_color,movement);
	}
	return fillet;
}
