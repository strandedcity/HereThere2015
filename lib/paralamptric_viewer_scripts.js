...
function drawModelGeometry(solar_tilt,lamp_height,draw_construction){
	...
	rib_object.geometry = joinGeometry([head.clone(),neck.clone(),back.clone(),tail.clone(),belly.clone(),throat.clone()]);
	var skin_length = 0;
...
function addSheet(sheet){
	...
	if (cut_sheets.length < 1){
		pt = new THREE.Vector3(cut_camera.left,cut_camera.top,0);
	} else {
