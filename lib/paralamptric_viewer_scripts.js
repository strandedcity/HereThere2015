...
var cut_renderer, cut_scene, cut_camera, cut_geometry;
...
function clearScene() {
	var obj, i;
	for (var i = model_scene.children.length - 1; i >= 0 ; i -- ) {
		obj = model_scene.children[i];
		proto = Object.getPrototypeOf(obj)
		if (proto !== THREE.PerspectiveCamera || proto !== THREE.PointLight) {
			model_scene.remove(obj);
		}
	}
	for (var i = cut_scene.children.length - 1; i >= 0 ; i -- ) {
		obj = cut_scene.children[i];
		proto = Object.getPrototypeOf(obj)
		if (proto !== THREE.PerspectiveCamera || proto !== THREE.PointLight) {
			cut_scene.remove(obj);
		}
	}
...
function drawCutGeometry(){
	...
	// var rect = getRectangle(pt,8,4);
	// drawLine(cut_scene,rect,"black");
	for (var i = 0; i < cut_geometry.length; i++){
		drawLine(cut_scene,cut_geometry[i],"black");
	}
	cut_renderer.render(cut_scene,cut_camera);
}

function drawModelGeometry(solar_tilt,lamp_height){
	...
	var skin_color = "gray";
	drawMesh(model_scene,extrudePiece(head,extrusion_distance,extrusion_direction),skin_color);
	drawMesh(model_scene,extrudePiece(neck,extrusion_distance,extrusion_direction),skin_color);
	drawMesh(model_scene,extrudePiece(body,extrusion_distance,extrusion_direction),skin_color);
	drawMesh(model_scene,extrudePiece(belly,extrusion_distance,extrusion_direction),skin_color);
	drawMesh(model_scene,extrudePiece(tail,extrusion_distance,extrusion_direction),skin_color);
	...
	cut_geometry = [head.clone(),neck.clone(),belly.clone(),tail.clone(),throat.clone(),back.clone()];
}
...
function getRectangle(sPt,width,height){
	var vertices = [sPt,sPt.clone().add(new THREE.Vector3(width,0,0)),sPt.clone().add(new THREE.Vector3(width,height,0)),sPt.clone().add(new THREE.Vector3(0,height,0)),sPt];
	var rect = new THREE.Geometry();
	rect.vertices.push.apply(rect.vertices, vertices);
	return rect;
}
