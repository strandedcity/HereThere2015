...
function clearScene(scene){
	for (var i = scene.children.length - 1; i >= 0 ; i -- ) {
		...
		if (!THREE.PerspectiveCamera.prototype.isPrototypeOf(obj) && !THREE.HemisphereLight.prototype.isPrototypeOf(obj)) {
			scene.remove(obj);
		}
	}
	animate();
...
function setupLights(scene){
	...
	var light = new THREE.HemisphereLight(0xffffff,0x8c8c8c,1);
...
function drawMesh(scene,geometry,color){
	...
	//var material = new THREE.MeshLambertMaterial({specular: 0xF5F5F5,color: new THREE.Color("white"),shininess: 5,side: THREE.DoubleSide});
